...
in
  {
    ...
    in
      ...
      then
        ...
        throw "Cannot merge definitions of `${
          showOption loc
        }', as at least one merge candidate is not a list of strings or a list of list of strings. Definition values:${
          showDefs defs
        }";
    ...
    harvesterType = mkOptionType {
      ...
      emptyValue = {value = [];};
      inherit
        (with lib.types; (either (listOf nonEmptyStr) (listOf (listOf nonEmptyStr))))
        check
        ;
    };
  ...
  in {
    ...
    options = {
      hive = {
        ...
        collect = mkOption {
          description = "Collect Hive outputs into a Nix-CLI-compatible form.";
          type = with lib.types; attersOf harvesterType;
          example = literalExpression ''
            {
              homeConfigurations = ["cell1" "homeConfigurations"];
              nixosConfigurations = [
                ["cell1" "nixosConfigurations"];
                ["cell2" "nixosConfigurations"];
              ];
            }
          '';
        };
    ...
    config = {
      flake = let
        ...
        collected = mapAttrs (_: v: collect grown v) cfg.collect;
        picked = mapAttrs (_: v: pick grown v) cfg.pick;
        harvested = mapAttrs (_: v: harvest grown v) cfg.harvest;
        winnowed = zipAttrsWith (n: v: winnow (head v) grown (head (tail v))) [
          cfg.winnowIf
          cfg.winnow
        ];
      in
        lib.mkIf opt.grow.isDefined (lib.foldl' lib.recursiveUpdate {} [grown]
          ++ (lib.optionals opt.collect.isDefined [collected])
          ++ (lib.optionals opt.pick.isDefined [picked])
          ++ (lib.optionals (opt.winnow.isDefined && opt.winnowIf.isDefined)
            [winnowed])
